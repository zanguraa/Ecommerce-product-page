...
function ImageBox() {
  ...
  const imageAlbum = imageArray.map((image, index) => {
    return (<img
      className=" h-[88px] w-[88px] hover:opacity-70 cursor-pointer opacity-50 border-2 border-orange-500 rounded-xl"
      src={image}
      key={index}
      alt="product1"
    />)
  ...
  return (
    <div className=" relative w-full max-w-[445px] flex flex-col gap-8">
      ...
      {imageAlbum }
        ...
        />
        <img
          className=" h-[88px] w-[88px] hover:opacity-70 cursor-pointer rounded-xl"
          src={product2}
          alt="product2"
        />
        <img
